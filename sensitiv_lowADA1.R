...
# low ADA-titre (<30 AU/mL)
# with memory T and no memory B cells
...
dxdt <- function(t, x, pars) {
  with(as.list(c(x,pars)), {
    ...
    dBa = aB_T*He/(He+b_n*Bsum)*B0 +s_m*r_Ba*(He/(He+b_n*Bsum))*Ba -mBa*Ba -r_Ps*Ba;
    
    dBm = r_m*(1-s_m)*(r_Ba*(He/(He+b_n*Bsum))*Ba + r_Bm*(He/(He+b_m*Bsum))*Bma) - aBm_T*He/(He+b_m*Bsum)*Bm + aBm_T*Bm*(1-Bm/KBm);
    
    dBma = aBm_T*He/(He+b_m*Bsum)*Bm +s_m*r_Bm*(He/(He+b_m*Bsum))*Bma-mBa*Bma;
    
    # short-lived plasma cells
    dPs = r_Ps*Ba -mPs*Ps;
    
	# long-lived plasma cells
    dP= (1-r_m)*(1-s_m)*(r_Ba*(He/(He+b_n*Bsum))*Ba 
...
y<- c(U=0, A=0, Ha=0, Hm=0,Hma=0,Ba=0,Bm=0,Bma=0,Ps=0, P=0, NK0=K_NK, NK1=6e4, D0=KD0, Dast=0,
      D1=0, H0= KH0, He=0, B0=KB0, TNFa=9.629e-6)
...
dev.off()
write.csv(as.data.frame(Sfun1),"sens_ADAlow1.csv")
